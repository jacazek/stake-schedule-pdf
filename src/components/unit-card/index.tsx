...
export const UnitCard: FC<PropsWithChildren & { unit: Unit }> = ({ unit }) => {
  ...
  const rowData = months.map((month) => ({
    month,
    stakeCounilSpeaker: unit.stakeCouncilSpeakers?.find(
      (x) => formatMonth(x.date) === month,
    ),
    providedSpeaker: unit.providingSpeakers?.find(
      (x) => formatMonth(x.date) === month,
    ),
    unitSpeaker: unit.receivingSpeakers?.find(
      (x) => formatMonth(x.date) === month,
    ),
    stakePresidencySpeaker: unit.stakePresidencySpeakers?.find(
      (x) => formatMonth(x.date) === month,
    ),
  }));
  let totalColumns = 1;
  ...
  const columnWidth = (1 - 0.13) / totalColumns;
    ...
    <View wrap={false}>
        ...
        {unit.conferenceDate ? (
          <>
            ...
            >
              ...
              >
                {new Date(unit.conferenceDate).toLocaleString("en-US", {
                  month: "long",
                  day: "numeric",
                  year: "numeric",
                })}
            ...
            <Link
              style={{ fontSize: "12px" }}
              src="https://docs.google.com/document/d/13rwCwdEoCrdJdrG6cnJ78cNzWcwUPCcJLRLWPYLpN4g/edit?usp=sharing"
            >
              Unit Conference Guide
            </Link>
          </>
        ) : null}
      </View>
      <Section title="Speaker Schedule">
        <Table
          ...
          weightings={[
            0.13,
            ...Array.from({ length: totalColumns }, () => columnWidth),
          ]}
          tdStyle={{
            padding: "4px",
          }}
        >
          <TableHeader style={{ backgroundColor: COLORS.TableHeader }}>
            ...
            {unit.receivingSpeakers?.length ? (
              <TableCell style={styles.headerCell}>
                <Text style={H4}>Unit Providing Speakers</Text>
              </TableCell>
            ) : undefined}
          </TableHeader>
          {rowData.map((row, index) => (
            <TableRow
              style={{ backgroundColor: index % 2 ? COLORS.Secondary : "" }}
            >
              <TableCell>
              ...
              {unit.providingSpeakers?.length ? (
                <TableCell style={styles.speakerCell}>
                  <Text style={STANDARD_FONT}>{row.providedSpeaker?.name}</Text>
                </TableCell>
              ) : undefined}
              {unit.receivingSpeakers?.length ? (
                <TableCell style={styles.speakerCell}>
                  <Text style={STANDARD_FONT}>{row.unitSpeaker?.name}</Text>
                </TableCell>
              ) : undefined}
            </TableRow>
          ))}
      ...
      {unit.leaderMeetings?.length > 0 && unit.leaderType ? (
        <MinisteringEvents
          title={`${unit.leaderType} - Ministering Interview`}
          interviews={unit.leaderMeetings}
          index={0}
        />
      ) : undefined}
      {unit.eqpMeetings?.length > 0 ? (
        <MinisteringEvents
          title="Elders Quorum President - Ministering Interview"
          interviews={unit.eqpMeetings}
          index={1}
        />
      ) : undefined}
    </View>

...
export const MinisteringEvents: FC<{ title: string, interviews: MinisteringEvent[], index: number }> = ({ index, title, interviews }) => {
        ...
        <Table
            ...
            {interviews.map((interview, interviewIndex) => (
                <TR key={`${index}-${interviewIndex}`} style={{ backgroundColor: interviewIndex % 2 ? COLORS.Secondary : "" }}>
                    ...
                    <TD>
                        <Text style={STANDARD_FONT}>{eventType[new Date(interview.date).getDay().toString()]}</Text>
                    </TD>
                    <TD>

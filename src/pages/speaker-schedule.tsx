...
export const SpeakerSchedule = () => {
  const { speakerData, loading, error } = useData();

  const sortedSpeakers = useMemo(() => {
    if (!speakerData) return [] as Speaker[];
    const arr = [...speakerData] as Speaker[];
    return arr.sort((a, b) =>
      a.tocName > b.tocName ? 1 : a.tocName < b.tocName ? -1 : 0,
    );
  }, [speakerData]);

  const dataKey = speakerData ? JSON.stringify(speakerData).length : 0;
            ...
            <Document>
              ...
              <Page size="A4" style={PAGE}>
                <View style={{ padding: ".5in" }}>
                  <SectionHeader
                    ...
                    tocName="Speaking Assignment Instructions"
                  >
                    <Text
                      style={{ textAlign: "center" }}
                      hyphenationCallback={(word) => [word]}
                    >
                      Instructions for SP, HC, and Stake Organization Speaking
                      Assignments
                    </Text>
                  </SectionHeader>
                </View>
                ...
                <View style={{ padding: "0 .75in" }}>
                  <Text style={PARAGRAPH}>
                    Stake Organizations should assign a member of their
                    presidency for the speaking assignment.
                  </Text>
                  <Text style={PARAGRAPH}>
                    Speakers should reach out to the bishop/ branch president 1
                    month before their assignment to coordinate the date and
                    topic.
                  </Text>
                  <Text style={PARAGRAPH}>
                    For months where multiple speakers are assigned you may
                    speak on the same day if the bishop/ branch president
                    approves and if your schedule permits.
                  </Text>
                </View>
                <PageNumber />
                <View fixed style={styles.backToMain}>
              ...
              <Page size="A4" style={PAGE}>
                <View style={{ padding: ".5in" }}>
                  <SectionHeader
                    id="speaking-topics"
                    tocName="Speaking Assignment Topics"
                  >
                    <Text
                      style={{ textAlign: "center" }}
                      hyphenationCallback={(word) => [word]}
                ...
                <View style={{ padding: "0 .75in" }}>
                  <Text style={PARAGRAPH}>
                    When scheduling your speaking assignment with a member of
                    the stake bishopric/branch presidency if they do not provide
                    you with a specific topic please prayerfully consider what
                    the members may need. You may also choose from the following
                    topics that the stake presidency has identified as
                    priorities. Focus on teaching eternal truths bearing
                    testimony of the Savior and extending inspired invitations.
                  </Text>
